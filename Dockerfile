...
RUN npm run build --workspace=frontend -- --base=/drawer-next/
...
# React candidate lives beside the legacy Drawer.
# It never owns /app/data and does not replace /drawer.
COPY --from=frontend-build /app/frontend/dist ./react-dist
